...
function loginCtrl($scope, $state, $timeout, $http, $window, toaster, commonServices, formEditService, fintaxService) {
        ...
        $http({
            ...
            if (response.data.length > 0) {
                ...
                for (var i = 0; i < resData.length; i++) {
                    ...
                    resData[i].percentAchievable = Math.round(resData[i].percentAchievable * 100);
                    if(resData[i].type == 'taxPlanning' || resData[i].type == 'contingency'){
                        resData[i].percentAchieved = resData[i].percentAchievable;
                    } else{
                        resData[i].percentAchieved = Math.round(resData[i].percentAchieved * 100);
                    }
...
function clientsCtrl($scope, $http, toaster, $rootScope, fintaxService) {
    $http({
        method: 'GET',
...
function serviceCartController($scope, $state, $http,$window,toaster,ModalService) {
    ...
    $scope.computedTotal = function () {
        $scope.totalAmount = 0;
        var selectFinComprehensive = false; /*FinComprehensive selected automatically*/
        var finComprehensiveSelected = false; /*FinComprehensive selected by user*/
        $scope.svcPurchased = [];
    ...
    $scope.checkout = function () {
        ...
        if($state.params.service) data.cookie = $state.params.service;

        angular.forEach($scope.fincarts, function (obj, index) {
            if (obj.selected == true && obj.offerPrice > 0) {
                if($scope.prefClient){
                    data.svcPurchased.push({name:obj.name,amount:obj.prefPrice, validity:moment().day(obj.validity)});
                } else {
...
function finTax($scope, $state, $uibModal, $http, fintaxService) {
    $scope.fintax = [
        {name: "Fintax Optimization", src: "/assets/img/taxOptimizationInfographic.jpg", type: "optimization"},
        {name: "Fintax Filing", src: "/assets/img/taxFilinginfographic.jpg", type: "filing"}
    ];
...
function rentInfoCtrl($scope, $state, fintaxService,convertDigitsIntoWords) {
    ...
    $scope.submitRentInfo = function () {
        fintaxService.setInfo('house', $scope.rentInfoFormData);
...
function healthInsuranceCtrl($scope, $state, fintaxService,convertDigitsIntoWords) {
    ...
    $scope.submitHealthInsurance = function () {
        ...
        $state.go("forms.fintax.otherInfo");
    };
 ...
 * Created by Finatwork on 31-03-2017.
 */
function otherInfoCtrl($scope, $state, fintaxService,convertDigitsIntoWords) {
    $scope.otherInfoFormData = {
        eduLoan: 'no',
    ...
    $scope.init = function () {
        fintaxService.getInfo().then(function (response) {
            var currentValue = response.other;
            if(currentValue){
                if (!IsAdmin())formStatus(response.state);
                if(currentValue.eduLoanIntAmt){
                    $scope.otherInfoFormData.eduLoan = 'yes';
                    $scope.otherInfoFormData.eduLoanIntAmt = currentValue.eduLoanIntAmt;
                }
                if(currentValue.disabilityInfo){
                    $scope.otherInfoFormData.dependentDisability = 'yes';
                    $scope.otherInfoFormData.disabilityInfo = currentValue.disabilityInfo;
                }
                if(currentValue.donationAmt){
                    $scope.otherInfoFormData.donation = 'yes';
                    $scope.otherInfoFormData.donationAmt = currentValue.donationAmt;
                }
            }
        }, function(reason){
            console.log(reason);
    ...
    $scope.otherInfo = function () {
        ...
        if ($scope.otherInfoFormData.eduLoan == 'yes') {
            otherInformation.eduLoanIntAmt = $scope.otherInfoFormData.eduLoanIntAmt;
        }
        if ($scope.otherInfoFormData.dependentDisability == 'yes') {
            otherInformation.disabilityInfo = $scope.otherInfoFormData.disabilityInfo;
        }
        if ($scope.otherInfoFormData.donation == 'yes') {
            otherInformation.donationAmt = $scope.otherInfoFormData.donationAmt;
        }

        fintaxService.setInfo('other', otherInformation);
        $state.go("forms.fintax.fintaxDoc_upload");
    };
    $scope.$watch('otherInfoFormData.eduLoanIntAmt', function (val) {
        if (val && val > 0) {
    ...
    $scope.$watch('otherInfoFormData.donationAmt', function (val) {
        if (val && val > 0) {
    ...
    function formStatus(state) {
        ...
        if (!$scope.formEditable) {
            $("#finTaxotherInfo :input").attr("disabled", true);
            $("#finTaxotherInfo :submit").attr("disabled", true);
        }
    }
}
angular.module('finatwork').controller('otherInfoCtrl', otherInfoCtrl);
...
function trxnStatusCtrl($scope, $http) {
    ...
    $scope.gridOptions2.columnDefs = [
        ...
        {name: 'Goal', "field": "_goalid", enableFiltering: false},
...
function filingPersonalInfoCtrl($scope, $state, fintaxFilingService, convertDigitsIntoWords, toaster) {
    $scope.init = function () {
        // $scope.personalInfoFormData = fintaxFilingService.getPersonalInfo();
...
function filingBankInfoCtrl($scope, $state, fintaxFilingService, convertDigitsIntoWords, toaster) {
    ...
    $scope.deleteBank = function (index) {
        for (i in $scope.bankList) {
            if ($scope.bankList[i]._id == index) {
...
function filingInvestmentCtrl($scope, $state, fintaxFilingService, fintaxService, convertDigitsIntoWords) {
    ...
    $scope.init = function () {
        fintaxFilingService.getInfo('investments').then(function (response) {
            if (response != null) {
                ...
                if ($scope.investmentInfo) {
                    //$scope.finTaxInvestmentFormData = currentVal;
    ...
    $scope.initiateInvestmentOptimization = function () {
        fintaxService.getInfo().then(function (response) {
            if (response != null) {
                $scope.investmentInfo = response.investments;
                if ($scope.investmentInfo) {
    ...
    $scope.submitInvstmnt = function () {
        ...
        fintaxFilingService.setInvestmentInfo(investmentInfo);//should be via promise
...
function filingHealthCtrl($scope, $state, fintaxFilingService, fintaxService, convertDigitsIntoWords) {
    ...
    $scope.init = function () {
        fintaxFilingService.getInfo('healthInsurance').then(function (response) {
            if(response != null){
                ...
                if (  $scope.healthInfo) {
                    //$scope.healthInsuranceFormData = currentValue;
    ...
    $scope.initiateHealthOptimization = function(){
        fintaxService.getInfo().then(function (response) {
            if(response != null){
                $scope.healthInfo = response.healthInsurance;
                if (  $scope.healthInfo) {
    ...
    $scope.submitHealthInfo = function () {
        ...
        fintaxFilingService.setHealthInsuranceInfo(healthInfo);//should be via promise
        $state.go("forms.returnFiling.filingOthers");
    }
    $scope.$watch('healthInsuranceFormData.corporateAmt', function (val) {
        if (val && val > 0) {
            $scope.corporateAmtInWords = convertDigitsIntoWords.inWords(val);
 ...
 */
function filingotherInfoCtrl($scope, $state, fintaxFilingService, fintaxService,convertDigitsIntoWords) {
    $scope.otherInfoFormData = {
        eduLoan: 'no',
        dependentDisability: 'no',
        donation: 'no'
    ...
    $scope.init = function () {
        fintaxFilingService.getInfo('other').then(function (response) {
            if(response != null){
                $scope.otherInformation = response.other;
                if($scope.otherInformation){
                    if($scope.otherInformation.eduLoanIntAmt){
                        $scope.otherInfoFormData.eduLoan = 'yes';
                        $scope.otherInfoFormData.eduLoanIntAmt = $scope.otherInformation.eduLoanIntAmt;
                    }
                    if($scope.otherInformation.disabilityInfo){
                        $scope.otherInfoFormData.dependentDisability = 'yes';
                        $scope.otherInfoFormData.disabilityInfo = $scope.otherInformation.disabilityInfo;
                    }
                    if($scope.otherInformation.donationAmt){
                        $scope.otherInfoFormData.donation = 'yes';
                        $scope.otherInfoFormData.donationAmt = $scope.otherInformation.donationAmt;
                    }
                }
            } else {
                console.log("employment empty");
    ...
    $scope.initiateOtherOptimization = function(){
        fintaxService.getInfo().then(function (response) {
            if(response != null){
                $scope.otherInformation = response.other;
                if($scope.otherInformation){
    ...
    $scope.otherInfo = function () {
        ...
        fintaxFilingService.setOtherInfo(otherInformation);
...
angular.module('finatwork').controller('filingotherInfoCtrl', filingotherInfoCtrl);
...
function filingOtherSrcInfoCtrl($scope, $state, fintaxFilingService, fintaxService, convertDigitsIntoWords) {
    ...
    $scope.submitOtherSrcInfo = function () {
        ...
        fintaxFilingService.setOtherSrcInfo(otherSourceInfo);
...
function filingCapitalGainsCtrl($scope, $state, fintaxFilingService,fintaxService, convertDigitsIntoWords) {
    $scope.capitalGainsFormData = {
        ...
        securities: 'no',
        propTransaction: 'no',
        propTransactionDetails:[{id: 0,propNature:'',propPurDate:'',propPurCost:'',improvementCost:'',propSaleDate:'',propSaleVal:'',costIncurred:''}]
    };
    $scope.isOpen = true;
    $scope.idToName = {
        0: "Property 1",
        ...
        fintaxFilingService.getInfo('capGain').then(function (response) {
            if (response != null && response.capGain !== undefined && response.capGain.investment != undefined) {
                ...
                if($scope.capitalGainsFormData.investment){
                    $scope.capitalGainsInfo = response.capGain;
                    if ($scope.capitalGainsInfo) {
                        if (response.capGain.shareLTCG) {
                            $scope.capitalGainsFormData.share = 'yes';
                            $scope.capitalGainsFormData.shareLTCG = $scope.capitalGainsInfo.shareLTCG;
                        }
                        if ($scope.capitalGainsInfo.shareSTCG) {
                            $scope.capitalGainsFormData.share = 'yes';
                            $scope.capitalGainsFormData.shareSTCG = $scope.capitalGainsInfo.shareSTCG;
                        }
                        if ($scope.capitalGainsInfo.mfLTCGEquity) {
                            $scope.capitalGainsFormData.mutualFund = 'yes';
                            $scope.capitalGainsFormData.mfLTCGEquity = $scope.capitalGainsInfo.mfLTCGEquity;
                        }
                        if ($scope.capitalGainsInfo.mfSTCGEquity) {
                            $scope.capitalGainsFormData.mutualFund = 'yes';
                            $scope.capitalGainsFormData.mfSTCGEquity = $scope.capitalGainsInfo.mfSTCGEquity;
                        }
                        if ($scope.capitalGainsInfo.mfLTCGDebt) {
                            $scope.capitalGainsFormData.mutualFund = 'yes';
                            $scope.capitalGainsFormData.mfLTCGDebt = $scope.capitalGainsInfo.mfLTCGDebt;
                        }
                        if ($scope.capitalGainsInfo.mfSTCGDebt) {
                            $scope.capitalGainsFormData.mutualFund = 'yes';
                            $scope.capitalGainsFormData.mfSTCGDebt = $scope.capitalGainsInfo.mfSTCGDebt;
                        }
                        if ($scope.capitalGainsInfo.securitiesBond) {
                            $scope.capitalGainsFormData.securities = 'yes';
                            $scope.capitalGainsFormData.securitiesBond = $scope.capitalGainsInfo.securitiesBond;
                        }
                        if ($scope.capitalGainsInfo.securitiesGold) {
                            $scope.capitalGainsFormData.securities = 'yes';
                            $scope.capitalGainsFormData.securitiesGold = $scope.capitalGainsInfo.securitiesGold;
                        }
                        if ($scope.capitalGainsInfo.securitiesOther) {
                            $scope.capitalGainsFormData.securities = 'yes';
                            $scope.capitalGainsFormData.securitiesOther = $scope.capitalGainsInfo.securitiesOther;
                        }
                        if($scope.capitalGainsInfo.propTransactionDetails.length != 0){
                            $scope.capitalGainsFormData.propTransactionDetails = $scope.capitalGainsInfo.propTransactionDetails;
                        }
                    }
                }
            } else {
    ...
    $scope.addCapitalInfo = function (capital) {
        var tmp = {propNature:'',propPurDate:'',propPurCost:'',improvementCost:'',propSaleDate:'',propSaleVal:'',costIncurred:''};
        tmp.id = capital.id + 1;
        $scope.capitalGainsFormData.propTransactionDetails.push(tmp);
        $scope.oneAtATime = true;
    };
    $scope.deleteCapitalInfo = function (index) {
        for (i in $scope.capitalGainsFormData.propTransactionDetails) {
            if ($scope.capitalGainsFormData.propTransactionDetails[i].id == index) {
                $scope.capitalGainsFormData.propTransactionDetails.splice(i, 1);
            }

        }
    };
    $scope.submitCapitalgains = function () {
        ...
        if($scope.capitalGainsFormData.propTransactionDetails.length != 0){
            capitalGainsInfo.propTransactionDetails = $scope.capitalGainsFormData.propTransactionDetails;
        }
        fintaxFilingService.setCapitalGainsInfo(capitalGainsInfo);
        $state.go("forms.returnFiling.filingOtherSources");
    };
...
function filingHouseCtrl($scope, $state, fintaxFilingService,fintaxService,convertDigitsIntoWords) {
    ...
    $scope.submitHouseProp = function () {
        fintaxFilingService.setHouseProperty($scope.propertyList);
        $state.go('forms.returnFiling.filingCapitalGains');
...
function filingSalaryIncomeCtrl($scope, $state, fintaxFilingService, fintaxService, convertDigitsIntoWords,ModalService,toaster) {
    $scope.salaryFormData = {
        salaried:'no',
        changedJob: 'no',
        prevEmploymentDetails : [{PrevEmployer:'',PrevEmploymentFromDate:'',PrevEmploymentEndDate:''}]
    };


    $scope.init = function () {
        // $scope.salaryFormData = fintaxFilingService.getInfo('salary');
        fintaxFilingService.getInfo('salary').then(function (response) {
            if(response != null){
                $scope.salaryFormData.salaried = 'yes';
                $scope.salaryFormData.changedJob = 'yes';
                $scope.salaryFormData = response.salary;
            } else {
                fintaxService.getBasicInfo().then(function (response) {
    ...
    $scope.addEmployer = function(index){
        $scope.salaryFormData.prevEmploymentDetails[1] = {PrevEmployer:'',PrevEmploymentFromDate:'',PrevEmploymentEndDate:''};
    };
    ...
    $scope.submitSalaryIncomeInfo = function () {
        fintaxFilingService.setSalaryIncome($scope.salaryFormData);
...
/*** Created by Nilabh on 15-05-2017.*/
function confirmationModalController($scope, close) {
    $scope.close = function (result) {
        close(result, 200);
    };
}
function docsTaxFilingCtrl($scope, $state, $http, $q, toaster, $uibModal, ModalService) {

    $scope.uploadAllowed = true;
    $scope.viewPan = "";
    $scope.viewForm26AS = "";
    $scope.viewPrevYearReturn = "";
    $scope.viewForm16 = "";
    $scope.viewInterestCert = "";
    $scope.viewCapGainShare = "";
    $scope.viewCapGainMF = "";
    $scope.viewCapGainOther = "";
    $scope.viewProofSec80C = "";
    $scope.viewProofSec80D = "";
    $scope.viewProofSec80G = "";
    $scope.viewProofSec80E = "";
