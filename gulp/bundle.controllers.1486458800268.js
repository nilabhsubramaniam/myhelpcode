...
function finHealthCtrl($scope, $http, toaster,$state) {
    ...
    $scope.showQuestion = function (q) {
        if (window.localStorage.getItem("finHealth_questions")) {
            var questions = JSON.parse(window.localStorage.getItem("finHealth_questions"));
            $scope.question_text = questions[q].question;
            $scope.answers = questions[q].answer;
            $scope.qno = q;
            $scope.category =questions[q].category;
        }
    };
    ...
    $scope.submitQuestion = function (question_text, qno) {
        ...
        if (selected >= 1) {
            $scope.dynamic = (qno+1) * 10;
            var resArray = [];
            resArray = (window.localStorage.getItem("finHealth_questions_answer")) ? JSON.parse(window.localStorage.getItem("finHealth_questions_answer")) : [];
            resArray.push(selected);
            window.localStorage.setItem("finHealth_questions_answer", JSON.stringify(resArray));
            ...
            if (window.localStorage.getItem("finHealth_questions")) {
                ...
                if (qno < questions.length) {
                    ...
                    var answerObj = {
                        ...
                        answer: JSON.parse(window.localStorage.getItem("finHealth_questions_answer"))
                    };
        ...
        $http({
            ...
            if (response.data != null) {
                ...
                $http({
                    ...
                    if (response.data != null) {
                        $scope.personalInfoFormData.NRI = response.data.NRI ? '1' : '0';
...
function basicInfo($scope, $http, toaster, $state, $window, $uibModal, commonServices, formEditService) {
    ...
    $scope.submitBasicInfoForm = function () {
       ...
       if($scope.basicInfoFormData.is_married === '1') {
        ...
        $http({
            ...
            if (response.data != null) {
                $scope.basicInfoFormData = {
                    ...
                    NRI: (response.data.NRI) ? '1' : '0',
                    is_married: (response.data.is_married) ? '1' : '0',
   ...
   //for Wealth Creation
    ...
    $scope.setMyGoal = function(){
        ...
        $http({
            ...
            $state.go("dashboards.goal_list",{showLumpsum:$scope.showLumpsum});
        }, function errorCallback(response) {
    ...
    function getBankDetails()  {
        $http({
            method: 'GET',
            url:  window.link.bank+"/"+window.localStorage.getItem("userid"),
            headers : { 'x-access-token': window.localStorage.getItem('token') }
        }).then(function successCallback(response) {
            if (response.data != null ){
                if(response.data.umrn || response.data.bankType === "billdesk") {
...
function goallistcontroller($scope, $state, $http ,toaster, fundData) {
    $scope.showLumpsum = $state.params.showLumpsum;
...
function fundlistcontroller($scope, $state, $http ,$window, fundData, c3ChartService, $q) {
    ...
    $scope.executeGoals = function(goalId){
        ...
        $http({
            ...
            var res = [{"state":"set","sip":22663,"lumpsum":0,"_goalid":"By_S9NzVg","netScore":4,"type":"marriage","assetAllocation":{"equityLarge":40,"equityMid":20,"debtLong":25,"debtShort":15,"gold":0,"liquid":0,"elss":0,"hybrid":0},"funds":[{"_schemeId":"FTI038","category":"EquityLarge","allocPercentage":40,"_userid":"H1xgpOSMC","_goalid":"By_S9NzVg","sip":9065,"sipRounded":9100},{"_schemeId":"FTI273","category":"EquityMid","allocPercentage":20,"_userid":"H1xgpOSMC","_goalid":"By_S9NzVg","sip":4533,"sipRounded":4500},{"_schemeId":"GMTG","category":"DebtShort","allocPercentage":15,"_userid":"H1xgpOSMC","_goalid":"By_S9NzVg","sip":3399,"sipRounded":3400},{"_schemeId":"B321G","category":"DebtLong","allocPercentage":25,"_userid":"H1xgpOSMC","_goalid":"By_S9NzVg","sip":5666,"sipRounded":5700}]}];
            var IsSipAvailable = false;
            var IsLampsumAvailable = false;
            if(res[0].funds[0].hasOwnProperty('sip') && res[0].funds[0].sip > 0)
                IsSipAvailable = true;
            if(res[0].funds[0].hasOwnProperty('lumpsum') && res[0].funds[0].lumpsum > 0)
                IsLampsumAvailable = true;
            $state.go("dashboards.nschandover", {sip: IsSipAvailable,lumpsum:IsLampsumAvailable});
        }, function errorCallback(response) {
            //toaster.error({body: response.data.err});
        });
    };
...
function nscHandOverCtrl($http, $scope, $state) {
    ...
    $scope.IsLampsumAvailable = $state.params.lumpsum;
    ($scope.IsSipAvailable === "true")? $scope.dynamicStep3 = "Step 3:" : $scope.dynamicStep3 = "Step 2:";
    if($scope.IsSipAvailable === "true" && $scope.IsLampsumAvailable === "true"){
        $scope.dynamicStep4 = "Step 4:";
    }else if($scope.IsSipAvailable === "true" || $scope.IsLampsumAvailable === "true"){
        $scope.dynamicStep4 = "Step 3:";
    }else{
        $scope.dynamicStep4 = "Step 2:";
    }
