 ...
 */
function serviceCartController($scope, $state, $http,$window) {
    $scope.init = function () {
        loadFinCart();
    };
    var loadFinCart = function () {
        $http({
            method: 'GET',
            url: window.StorageURL.fincarts
        }).then(function successCallback(response) {
    ...
    $scope.computedTotal = function () {
        ...
        angular.forEach($scope.fincarts, function (obj, index) {
            if (obj.selected == true) {
                $scope.total += obj.offerPrice;
                $scope.svcPurchased.push({name:obj.code,amount:obj.offerPrice});
            }
        });
        return $scope.total;
    };
    ...
    $scope.checkout = function () {
        ...
        data.amount = $scope.total;
        data.svcPurchased = $scope.svcPurchased;
        $http({
            method: 'POST',
            url: window.link.pg,
            data: data,
...
function pgResponseController($scope, $state, $stateParams, $http) {
    ...
    $scope.validTrxn = false;

    $http({
        method: 'GET',
        url: window.link.pg + '/' + $stateParams.token
    }).then(function successCallback(response) {
        console.log(response);
    }, function errorCallback(response) {
        $state.go("reset");
