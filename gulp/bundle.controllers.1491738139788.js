...
function navigationCtrl($location, $scope, $window, $rootScope, commonServices,fintaxService,$state) {
    ...
    $scope.init = function () {
        console.log("navigationCtrl init()");
        fintaxService.initialiseSubscription();
        fintaxService.initialiseFinTaxOpt();
    };

    $scope.init();

    $scope.isEdit = commonServices.getIsEditable();
    //fintaxService.getSubscriptionStatus();
    ...
    $scope.openFinTax = function () {
        ...
        if (fintaxService.getSubscriptionStatus() == true) {
            if (fintaxService.getBasicInfo() != null) {
...
function serviceCartController($scope, $state, $http,$window,toaster,ModalService) {
    ...
    $scope.computedTotal = function () {
        ...
        $scope.onPrefClientChange = function () {
...
function finTax($scope, $state, $uibModal, $http, fintaxService) {
    $scope.fintax = [
        {name: "Fintax Optimization", src: "/assets/img/tax_optimization.jpg", type: "optimization"},
        {name: "Fintax Filing", src: "/assets/img/tax_filing.jpg", type: "filing"}
    ];
 ...
 */
function rentInfoCtrl($scope, $state, fintaxService) {
    $scope.rentInfoFormData = fintaxService.getRentInfo();
    $scope.submitRentInfo = function () {
        fintaxService.setRentInfo($scope.rentInfoFormData);
 ...
 */
function invstmntFinTaxCtrl($scope, $state, fintaxService) {
    ...
    $scope.init = function () {
        var currentVal = fintaxService.getInvestmentInfo();
        if (currentVal) {
    ...
    $scope.submitInvstmntInfo = function () {
        ...
        // if($scope.finTaxInvestmentFormData.other == 'no' ){
        //     $scope.finTaxInvestmentFormData.otherInvst = 0;
        // }
        fintaxService.setInvestmentInfo(investmentInfo);
...
function healthInsuranceCtrl($scope, $state, fintaxService) {
    ...
    $scope.init = function () {
        var currentValue = fintaxService.getHealthInsuranceInfo();
        if (currentValue) {
    ...
    // $scope.healthInsuranceFormData = fintaxService.getHealthInsuranceInfo();
    $scope.submitHealthInsurance = function () {
        var healthInfo = {};
        if ($scope.healthInsuranceFormData.corporateHealth == 'yes') {
            healthInfo.corporateAmt = $scope.healthInsuranceFormData.corporateAmt;
        }
        if ($scope.healthInsuranceFormData.parentsInsurance == 'yes') {
            healthInfo.parentsAmt = $scope.healthInsuranceFormData.parentsAmt;
        }
        if ($scope.healthInsuranceFormData.selfInsurance == 'yes') {
            healthInfo.privateAmt = $scope.healthInsuranceFormData.privateAmt;
        }
        if ($scope.healthInsuranceFormData.parentPersonalInsurance == 'yes') {
            healthInfo.pvtParentsAmt = $scope.healthInsuranceFormData.pvtParentsAmt;
        }
        if ($scope.healthInsuranceFormData.illnessInsurance == 'yes') {
            healthInfo.criticalIllnessAmt = $scope.healthInsuranceFormData.criticalIllnessAmt;
        }
        fintaxService.setHealthInsuranceInfo($scope.healthInsuranceFormData);
 ...
 * Created by Finatwork on 31-03-2017.
 */
function otherInfoCtrl($scope, $state, fintaxService) {
    ...
    $scope.init = function () {
        var currentValue = fintaxService.getOtherInfo();
        if(currentValue){
    ...
    $scope.otherInfo = function () {
        ...
        fintaxService.setOtherInfo($scope.otherInfoFormData);
 ...
 */
function housePropTaxCtrl($scope, $state, fintaxService) {
    var uid = 1;
    $scope.propertyList = [];
    $scope.AddProp = true;
    $scope.formStatus = false;
    $scope.init = function () {
        fintaxService.getBasicInfo();
        if (fintaxService.getHouseProperty() != null) {
            $scope.propertyList = fintaxService.getHouseProperty();
            $scope.AddProp = false;
        }
    };
    ...
    //useless code..neeed to removed => for checking 3 prooperties
    // $scope.chk_allocation = function () {
    //     var propertyList = $scope.propertyList;
    //     if($scope.propertyList>=3){
    //         console.log("correct")
    //     }
    // };
    $scope.submitHouseProp = function () {
        if ($scope.formStatus) {
            ...
            /*add toaster info cannt be changed*/
        } else {
            // if( chk_allocation()){
            //     toaster.error({body: "We allow up to 3 properties"}); //need to ask why only 3 prop,why  not 4 or higher
            // }
            // for (var i = 0; i < $scope.propertyList.length; i++) {
            //     if (Number.isInteger($scope.propertyList[i]._id)) {
            //         delete $scope.propertyList[i]._id;
            //     }
            // }
            fintaxService.setHouseProperty($scope.propertyList);
            $state.go('forms.fintax.rent');
        }
    }
...
function basicInfoTaxCtrl($scope, $state, fintaxService) {
    ...
    $scope.basicInfoTaxFormData = fintaxService.getBasicInfo();
...
function docsFinTaxOptCtrl($scope, $state, $http, $q, toaster, $uibModal) {
    ...
    function currentValue() {
        $http({
            ...
            if (response.data != null) {
                angular.forEach(response.data.doc, function (docs) {
                    if (docs.doc_type === "payslip") {
                        $scope.viewPayslip = google + docs.s3_url_data;
                        $scope.payslipLinkExists = true;
                    }
                    if (docs.doc_type === "ctc") {
                        $scope.viewAddress = google + docs.s3_url_data;
                        $scope.ctcLinkExists = true;
                    }
                    if (docs.doc_type === "taxstmt") {
                        $scope.viewCancelledcheque = google + docs.s3_url_data;
