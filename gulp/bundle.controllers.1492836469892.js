...
function housePropTaxCtrl($scope, $state, fintaxService,convertDigitsIntoWords) {
    ...
    $scope.init = function () {
        $scope.basicInfo = fintaxService.getBasicInfo();
        if (fintaxService.getHouseProperty().length != 0) {
            $scope.propertyList = fintaxService.getHouseProperty();
            $scope.AddProp = false;
            $scope.housePropertyData.houseProp = 'yes';
        } else {
            $scope.housePropertyData.houseProp = $scope.basicInfo.houseProp;
        }
    };
    ...
    $scope.housePropUI = function (type) {
        var display = {
            ...
            'rented': 'Rented',
            'underConstruction': 'Under contruction'
        };
        return display[type];
    }
