...
function goalDashboardCtrl($scope, DTOptionsBuilder, $http, NotifyingService, thousandSeparator, goalData, $state, toaster) {
    ...
    function getGoalsList(status) {
        ...
        if(IsAdmin()){
            url = window.link.create_goal_admin + "?state=" + status + "&view=goal&userid=" + window.currentUesrId;
        }
        $http({
            method: 'GET',
            url: url,
    ...
    $scope.commitGoal = function (goalId){
        ...
        if(IsAdmin()){
            url = window.link.create_goal_admin + "/" +goalId+"?view=goal&userid=" + window.currentUesrId;
        }
        $http({
            method: 'GET',
            url:  url,
...
function goallistcontroller($scope, $state, $http ,toaster, fundData) {
    ...
    $scope.init = function(){
        ...
        if(IsAdmin()){
            url = window.link.create_goal_admin+"?state=inactive&view=goal&userid=" + window.currentUesrId;
...
function goalWiseDashboardCtrl($scope, $state, $http, DTOptionsBuilder, thousandSeparator, toaster) {
    ...
    $scope.getGoalsList = function(status){
        ...
        $http({
            method: 'GET',
            url:  window.link.create_goal+"?state="+status+"&view=goal+fund",
            headers : { 'x-access-token': window.localStorage.getItem('token') }
        }).then(function successCallback(response) {
            if (response.data.length > 0) {
                resData = response.data;
...
function finHealthCtrl($scope, $http, toaster, $state, $timeout) {
    ...
    $scope.overAllScoreValidate = function () {
        ...
        if (len === 15) {
            ...
            toaster.error({body: "Please answer all the question"});
            $timeout(function () {
...
function goalPortfolioDashboardCtrl($scope, $state, $http, c3ChartService, thousandSeparator, toaster) {
    ...
    $scope.gridOptions.columnDefs = [
        ...
        { name: 'Folio',field:"Folio",width: '13%'},
    ...
    function showAMCChart(){
        ...
        for(var i in $scope.fundAll){
            ...
            for (var k=0; k<$scope.fundAll[i].trxn.length; k++) {
                $scope.fundAll[i].trxn[k].tradDate =  moment($scope.fundAll[i].trxn[k].tradDate, "YYYY-MM-DD").format('YYYY-MM-DD');
    ...
    function getFundData(){
        $http({
            method: 'GET',
            url:  window.link.create_goal+"?state=active&view=goal+fund+trxn",
            headers : { 'x-access-token': window.localStorage.getItem('token') }
        }).then(function successCallback(response) {
            if (response.data.length > 0) {
