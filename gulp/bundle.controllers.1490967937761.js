...
function navigationCtrl($location, $scope, $window, $rootScope, commonServices) {
...
function loginCtrl($scope, $state, $timeout, $http, $window, toaster, commonServices, formEditService) {
...
function nomineeInfo($scope, $http, toaster, formEditService, Scopes, $state) {
    ...
    $scope.chk_allocation = function () {
        ...
        return (totalPercent === 100) ? false : true;
    }
            ...
            if (response.data != null) {
                for (var i = 0; i < response.data.details.length; i++) {
                    ...
                    if (response.data.details[i].city === response.data.details[i].state) {
                        response.data.details[i].city = response.data.details[i].city;
                    } else {
...
function goalDashboardCtrl($scope, DTOptionsBuilder, $http, NotifyingService, thousandSeparator, goalData, $state, toaster) {
    $scope.formStatus = false;
    var resData;
    ...
    function getGoalsList(status) {
        ...
        var url = window.link.create_goal + "?state=" + status + "&view=goal";
        if(IsAdmin()){
            url = window.link.create_goal + "?state=" + status + "&view=goal&userid=" + window.currentUesrId;
        }
        $http({
            method: 'GET',
            url: url,
            ...
            if (response.data.length > 0) {
                ...
                for (var i = 0; i < resData.length; i++) {
                    resData[i].updatedAt = moment(resData[i].updatedAt).format('DD-MM-YYYY  A');
                    resData[i].createdAt = moment(resData[i].createdAt).format('DD-MM-YYYY  A');
    ...
    function drawSeriesChart() {
        ...
        var minY = d3.min(resData, function(d) { return d.marketValue; });
        if(minY<10000) minY = 10000;
        var options = {
            title: 'Goal Dashboard',
            vAxis: {title: 'Portfolio Value',
                minValue:minY,
                maxValue:d3.max(resData, function(d) { return d.marketValue; })},
            hAxis: {title: 'Goal Maturity Year',
                minValue:d3.min(resData, function(d) { return d.maturity; }),
                maxValue:d3.max(resData, function(d) { return d.maturity; }),
                format:''},
    ...
    $scope.commitGoal = function (goalId){
        var url = window.link.create_goal+"/"+goalId+"?view=goal";
        if(IsAdmin()){
            url = window.link.create_goal + "/" +goalId+"?view=goal&userid=" + window.currentUesrId;
        }
        $http({
            method: 'GET',
            url:  url,
    ...
    $scope.revisitGoal = function (goalId){
...
angular.module('finatwork').controller('goalDashboardCtrl', goalDashboardCtrl);
/**
 * NomineeInfo - Controller for NomineeInfo Form
 */
...
function goalInfo($scope, $state, $http ,$window, c3ChartService, convertDigitsIntoWords, thousandSeparator, goalData, $uibModal, $rootScope, toaster) {
    ...
    if($state.params.type === "contingency")
        $scope.goalLabel = {
            ...
            featurecost:"Contingency Corpus Reqd",
            corpusLabel:"Corpus required for downpayment ",
            investmentLabel:"Monthly Investment required ",
    ...
    if($state.params.type === "retirement") {
        $scope.goalLabel = {
            ...
            investmentLabel:"Monthly Investment required",
            investmenttooltip:"You could exclude the invesment you are already making through other Retirement avenues (EPF, NPS, SA etc)",
    ...
    if($state.params.type === "education" )
        $scope.goalLabel = {currentprice:"Current Cost of Goal",year:"Number of Years ",percentage:"",isRequiredGoalByName:true,isRequiredPercentage:false,goalNamePlaecHolder:"Personalise your goal by naming it",currentPricePlaecHolder:" Current cost of education",yearPlaecHolder:"years to achieve this goal",loanPercentagePlaecHolder:"Amount",amountMaxLength:2,todaycost:"Today's cost",featurecost:"Future Cost",corpusLabel:"Corpus required for downpayment ",investmentLabel:"Monthly Investment required ",investmenttooltip:"Monthly Investment required for goal achievement ",guagenote:"",areachartLabel:"Additional returns over Fixed Deposit in a Realistic scenario : ",accumulateText:"Accumulate"};
    if($state.params.type === "other")
        $scope.goalLabel = {currentprice:"Current Cost of Goal",year:"Number of Years ",percentage:"",isRequiredGoalByName:true,isRequiredPercentage:false,goalNamePlaecHolder:"Personalise your goal by naming it",currentPricePlaecHolder:" Current value of your goal",yearPlaecHolder:"years to achieve this goal",loanPercentagePlaecHolder:"Amount",amountMaxLength:2,todaycost:"Today's cost",featurecost:"Future Cost",corpusLabel:"Corpus required for downpayment ",investmentLabel:"Monthly Investment required ",investmenttooltip:"Monthly Investment required for goal achievement ",guagenote:"",areachartLabel:"Additional returns over Fixed Deposit in a Realistic scenario : ",accumulateText:"Accumulate"};
   //for crorepati
    if($state.params.type === "crorepati")
        $scope.goalLabel = {currentprice:"Amount",year:"Number of Years ",isRequiredGoalByName:true,goalNamePlaecHolder:"Personalise your goal by naming it",currentPricePlaecHolder:" How many crores would you like to accumulate?",yearPlaecHolder:"years to achieve this goal",amountMaxLength:2,todaycost:"Today's cost",featurecost:"Future Cost",corpusLabel:"Corpus required for downpayment ",investmentLabel:"Monthly Investment required ",investmenttooltip:"Monthly Investment required for goal achievement ",guagenote:"",areachartLabel:"Additional returns over Fixed Deposit in a Realistic scenario : ",accumulateText:"Accumulate"};
   //for Wealth Creation
    ...
    $scope.submitGoalInfoForm = function(){
        ...
        if($scope.type === "taxPlanning"){
            $scope.goalInfoFormData_goalName ="Tax Planning (2016-17)";
        }
        ...
        //data._userid = (IsAdmin()) ? window.currentUesrId : window.localStorage.getItem("userid");
        ...
        if($scope.type === "crorepati") {
            //$scope.goalInfoFormData_crorepaticurrentprice= $scope.crorepatiSliderOptions.value;
        ...
        $http({
          ...
          headers : {'Content-Type': 'application/x-www-form-urlencoded'}
            ...
            if($state.params.type === "taxPlanning"){
                $scope.lumpsumAmount = response.data.lumpsumRequired;
                $scope.futurePrice = response.data.lumpsumRequired;
                $scope.resMonthlyInvestment = response.data.lumpsumRequired;
            ...
            if($scope.type === "wealthCreation"){
                var lumpsum = "";
                if($scope.showLumpsum) lumpsum = "&" +" " + $scope.goalInfoFormData_lumpsum + " " + "as lumpsum";

                $scope.goalLabel.accumulateText =
                 "Invest " +  $scope.thousandseparator($scope.monthlyInvestment) + " per month " + lumpsum + " until " + $scope.accumulatedYear;
            }
        }, function errorCallback(response) {
            toaster.error({body:response.data.err});
        });
    ...
    $scope.next = function(){
        ...
        var types = {};
        if($state.params.type === "taxPlanning"){
            types = {"PPF":"area-spline","Optimist":"area-spline","Pessimist":"area-spline","Real":"area-spline"};
        }else{
    ...
    $scope.setMyGoal = function(){
        ...
        if($scope.type === "taxPlanning") {
            if (( lumpmsumAmount > $scope.currentPrice)) {
...
function goallistcontroller($scope, $state, $http ,toaster, fundData) {
    ...
    $scope.init = function(){
        ...
        var url = window.link.create_goal+"?state=inactive&view=goal";
        if(IsAdmin()){
            url = window.link.create_goal+"?state=inactive&view=goal&userid=" + window.currentUesrId;
            data._userid = window.currentUesrId;
        }
        $http({
            method: 'GET',
            url:  url,
    ...
    $scope.isEditable = function (index) {
        if ($scope.goallists[index].isEditable == true) {
            $scope.goallists[index].isEditable = false;
        } else {
            $scope.goallists[index].isEditable = true;
        }

        if ($scope.goallists[index].isLumpsumEditable == true) {
            $scope.goallists[index].isLumpsumEditable = false;
        } else {
            $scope.goallists[index].isLumpsumEditable = true;
        }

        if($scope.goallists[index].type === "taxPlanning"){
            $scope.goallists[index].isSIPEditable = false;
        }else{
            if ($scope.goallists[index].isSIPEditable == true) {
                $scope.goallists[index].isSIPEditable = false;
            } else {
                $scope.goallists[index].isSIPEditable = true;
            }
        }
    ...
    $scope.Submit = function () {
        ...
        angular.forEach($scope.goallists, function (goallist) {
            if (goallist.isUpdated) {
                var sipRequired = 0;
                //(goallist.sip === 0)? sipRequired = goallist.sipRequired : sipRequired = goallist.sip;
                dataListToUpdate.push({state: "set", sip: parseInt(goallist.sip), lumpsum: parseInt(goallist.lumpsum), _goalid: goallist._goalid, netScore: goallist.netScore, type: goallist.type});
            }
        });
        var data = {
            _userid: (IsAdmin()) ? window.currentUesrId : window.localStorage.getItem("userid"),
            "state":"set",
            goals:dataListToUpdate
        };
        if(dataListToUpdate.length <= 0){
    ...
    $scope.updateGoal = function(index){
        ...
        if(type === "taxPlanning") {
            var CurrentPrice = $scope.goallists[index].currentPrice;
            if (( lumpmsumAmount > CurrentPrice)) {
                toaster.error({body: "There is no additional tax benefit of investing more than the shortfall under 80C. Investment in ELSS have a lock-in of three years, suggest you invest the balance amount in another goal."});
                $scope.goallists[index].lumpsum =  CurrentPrice ;
                $scope.goallists[index].isEditable = false;
                $scope.goallists[index].isLumpsumEditable = false;
                return;
            }
    ...
    function fourmStatus() {
        var url =  window.link.engagement + "/" + window.localStorage.getItem("userid");
        if(IsAdmin()){
            url= window.link.engagement + "/" + window.currentUesrId;
        }
        $http({
            method: 'GET',
            url: url,
...
function fundlistcontroller($scope, $state, $http ,$window, fundData, c3ChartService, $q) {
    ...
    var loadSchemeallocation = function() {
        $http({
            ...
            var fundAll = fundData.getFundData();
            for(var i in fundAll) {
                var lampsumAvailable = false, sipAvailable = false,sipArray = [],lampsumArray = [];
                for (var k = 0; k < fundAll[i].funds.length; k++) {
                    fundAll[i].funds[k].schemename = $scope.getFundName(fundAll[i].funds[k]._schemeId);
                    if(fundAll[i].funds[k].lumpsum){
                        lampsumAvailable = true;
                        lampsumArray.push(fundAll[i].funds[k]);
                    }
                    if(fundAll[i].funds[k].sip){
                        sipAvailable = true;
                        sipArray.push(fundAll[i].funds[k]);
                    }
                }
                fundAll[i].lampsumAvailable = lampsumAvailable;
                fundAll[i].sipAvailable = sipAvailable;
                fundAll[i].sipArray = sipArray;
                fundAll[i].lampsumArray = lampsumArray;
            }

            $scope.fundList = fundAll;
        }, function errorCallback(response) {

        });
    };
    $scope.dynamicChartId = 'pie-plot1-chart';
    //$scope.dynamicFundChartId = 'pie-fund-chart';
    $scope.dynamicSIPChartId = 'pie-sip-chart';
    //$scope.dynamicSIP1ChartId = 'pie-sip1-chart';
    $scope.chartType = {};
    $scope.funds = [];
    $scope.data = {
        columns: chartData,
    ...
    $scope.executeGoals = function(goalId){
        //var dataListToUpdate = [{state:"execute",_goalid:goalId}];
        //dataListToUpdate.push({state:"execute",_goalid:goallist._goalid});
        /*angular.forEach($scope.fundList, function (goallist) {
            dataListToUpdate.push({state:"execute",_goalid:goallist._goalid});
        }); */
        var data = {
            _userid: window.localStorage.getItem("userid"),
        ...
        $http({
            ...
            $state.go("dashboards.transcation-view");
        });
    };
...
function goalWiseDashboardCtrl($scope, $state, $http, DTOptionsBuilder, thousandSeparator, toaster) {
    ...
    $scope.getGoalsList = function (status) {
        ...
        $http({
            method: 'GET',
            url: window.link.create_goal + "?state=" + status + "&view=goal+fund&userid=" + userId,
            headers: {'x-access-token': window.localStorage.getItem('token')}
        }).then(function successCallback(response) {
            if (response.data.length > 0) {
                resData = response.data;
...
function healthResultCtrl($scope, $state, $http, $timeout) {
    ...
    $http({
        method: 'GET',
        url: window.link.healthProfileResult + "/" + window.localStorage.getItem("userid"),
        headers: {'x-access-token': window.localStorage.getItem('token')}
    }).then(function successCallback(response) {
        if (response.data == null) {
...
function finHealthCtrl($scope, $http, toaster, $state, $timeout) {
    ...
    $scope.questionsData = [{
        "_id": "57df8b4a49374502f4ac109e",
        "updatedAt": "2016-10-18T06:47:18.181Z",
        "createdAt": "2016-09-19T06:52:58.817Z",
        "qsno": 1,
        "question": "What percentage of your take home income do you save?",
        "category": "Savings Habit",
        "__v": 0,
        "answer": [{"option": "I manage to save more than 25% of my income."}, {"option": "I save 10-20% of my income."}, {"option": "I’m not in a position to save anything from my income."}]
    }, {
        "_id": "57df8758ee55881074c0fb4b",
        "updatedAt": "2016-10-18T06:48:14.480Z",
        "createdAt": "2016-09-19T06:36:08.202Z",
        "qsno": 2,
        "question": "Are you able to stick to your monthly expense budget?",
        "category": "Savings Habit",
        "__v": 0,
        "answer": [{"option": "Yes, I record my expenses and track them with my budget."}, {"option": "Yes, more or less but I don’t keep close track of it."}, {"option": "Budget? What's that? I spend as and how I feel comfortable."}]
    }, {
        "_id": "57df8758ee55881074c0fb4d",
        "updatedAt": "2016-10-18T06:39:58.512Z",
        "createdAt": "2016-09-19T06:36:08.235Z",
        "qsno": 3,
        "question": " Do you invest the money that you save or just let it lie idle in your savings account?",
        "category": "Savings Habit",
        "__v": 2,
        "answer": [{"option": "I have an investment plan and I make my monthly investments diligently."}, {"option": "I do not have an investment plan but I do tend to invest once some amount is accumulated."}, {"option": "Money is safest in my bank account, so I just let it lie there."}]
    }, {
        "_id": "57df8758ee55881074c0fb4e",
        "updatedAt": "2016-10-18T06:40:47.306Z",
        "createdAt": "2016-09-19T06:36:08.246Z",
        "qsno": 4,
        "question": " How many months would you be able to survive in case of a job loss?",
        "category": "Contingency Preparedness",
        "__v": 0,
        "answer": [{"option": "More than 6 months."}, {"option": "3 to 6 months."}, {"option": "Less than 3 months."}]
    }, {
        "_id": "57df8758ee55881074c0fb4f",
        "updatedAt": "2016-10-18T06:41:28.019Z",
        "createdAt": "2016-09-19T06:36:08.256Z",
        "qsno": 5,
        "question": " Is your family (spouse/parents) financially dependent on you?",
        "category": "Contingency Preparedness",
        "__v": 0,
        "answer": [{"option": "I don’t have any financial dependants."}, {"option": "My parents are financially independent but my spouse is dependent on me."}, {"option": "Both my parents and spouse are dependent on me."}]
    }, {
        "_id": "57df8758ee55881074c0fb50",
        "updatedAt": "2016-10-18T06:42:18.638Z",
        "createdAt": "2016-09-19T06:36:08.266Z",
        "qsno": 6,
        "question": "  Do you have life and health insurance over and above what your company offers?",
        "category": "Contingency Preparedness",
        "__v": 0,
        "answer": [{"option": "Yes, I think I’m adequately insured for the risks I’m exposed to."}, {"option": "I do have some insurance apart from the company cover, but I don’t know if it is adequate."}, {"option": "No, the only insurance I have is the one provided by my employer."}]
    }, {
        "_id": "57df8758ee55881074c0fb51",
        "updatedAt": "2016-10-18T06:43:01.306Z",
        "createdAt": "2016-09-19T06:36:08.280Z",
        "qsno": 7,
        "question": " What percentage of your income goes towards servicing your EMI’s?",
        "category": "Loan situation",
        "__v": 0,
        "answer": [{"option": "Less than 20%."}, {"option": "About 20% to 30%."}, {"option": "More than 30%."}]
    }, {
        "_id": "57df8758ee55881074c0fb52",
        "updatedAt": "2016-10-18T06:43:42.710Z",
        "createdAt": "2016-09-19T06:36:08.292Z",
        "qsno": 8,
        "question": "  Do you have any personal loan or credit card debt?",
        "category": "Loan situation",
        "__v": 0,
        "answer": [{"option": "No, I don’t have either personal loan or debt on my credit card."}, {"option": "I pay off my credit card regularly but I do have a personal loan."}, {"option": "Yes, I do have debt on my credit card."}]
    }, {
        "_id": "57df8758ee55881074c0fb53",
        "updatedAt": "2016-10-18T06:44:02.709Z",
        "createdAt": "2016-09-19T06:36:08.302Z",
        "qsno": 9,
        "question": "How do you generally make big ticket expenses?",
        "category": "Loan situation",
        "__v": 0,
        "answer": [{"option": "I plan for them and start saving in advance."}, {"option": "I avoid using my credit card or taking a loan but I may dip into my savings."}, {"option": "I usually take a loan or use my credit card."}]
    }, {
        "_id": "57df8758ee55881074c0fb54",
        "updatedAt": "2016-10-18T06:44:41.214Z",
        "createdAt": "2016-09-19T06:36:08.314Z",
        "qsno": 10,
        "question": "Have you set (smart) financial goals in life?",
        "category": "Investment Discipline",
        "__v": 0,
        "answer": [{"option": "Yes, I know my future goals as well as how much I will require to fulfill them."}, {"option": "I have it in my mind but I haven’t put it in black and white."}, {"option": "No, I haven't."}]
    }, {
        "_id": "57df8758ee55881074c0fb55",
        "updatedAt": "2016-10-18T06:45:00.114Z",
        "createdAt": "2016-09-19T06:36:08.326Z",
        "qsno": 11,
        "question": "How much % of your income goes towards investments?",
        "category": "Investment Discipline",
        "__v": 0,
        "answer": [{"option": "More than 20%."}, {"option": "About 10% to 20%."}, {"option": "I’m not investing currently because I don’t think I have any savings."}]
    }, {
        "_id": "57df8758ee55881074c0fb56",
        "updatedAt": "2016-10-18T06:45:30.508Z",
        "createdAt": "2016-09-19T06:36:08.334Z",
        "qsno": 12,
        "question": "Amongst the following where does your maximum investment go?(FD, PPF, Gold, Real Estate, Mutual Funds)",
        "category": "Investment Discipline",
        "__v": 0,
        "answer": [{"option": "I have a good mix of investments in my investment portfolio."}, {"option": "I usually end up investing in fixed deposits and PPF but I want to start investing in mutual funds and shares."}, {"option": "I invest only in fixed deposits, real estate or gold."}]
    }, {
        "_id": "57df8758ee55881074c0fb57",
        "updatedAt": "2016-10-18T06:49:23.668Z",
        "createdAt": "2016-09-19T06:36:08.348Z",
        "qsno": 13,
        "question": "Are you aware of your employment benefits –EPF, Superannuation, NPS?",
        "category": "General Awareness",
        "__v": 0,
        "answer": [{"option": "Yes, I know about these benefits and how they work."}, {"option": "I know about EPF but don’t know the rest of them."}, {"option": "I do know that I get some benefits but I have no idea how they work."}]
    }, {
        "_id": "57df8758ee55881074c0fb58",
        "updatedAt": "2016-10-18T06:46:22.189Z",
        "createdAt": "2016-09-19T06:36:08.364Z",
        "qsno": 14,
        "question": "Is your spouse aware of all your investments, insurance etc.? Are they prepared to manage finances in your absence?",
        "category": "General Awareness",
        "__v": 0,
        "answer": [{"option": "Yes, very much."}, {"option": "Yes but I’m unsure if they will be able to manage without me."}, {"option": "No, my spouse has no clue!"}]
    }, {
        "_id": "57df8758ee55881074c0fb59",
        "updatedAt": "2016-10-18T06:48:53.755Z",
        "createdAt": "2016-09-19T06:36:08.386Z",
        "qsno": 15,
        "question": "Do you have a Will in place?",
        "category": "General Awareness",
        "__v": 0,
        "answer": [{"option": "Yes, I do have a Will or I’m planning to get it done soon."}, {"option": "Do I need a Will? If yes, I would be wiling to have one made."}, {"option": "I don’t think I need a Will until I’m 60."}]
    }];
    ...
    function postAnswer(answer) {
        ...
        $http({
            ...
            window.localStorage.setItem("finHealth_questions_answer", "");
    ...
    $scope.submitQuestion = function (questions) {
        ...
        if (questions === "savingsHabit") {
            var ques1 = $scope.Answer.hasOwnProperty($scope.category1[0]._id);
            var ques2 = $scope.Answer.hasOwnProperty($scope.category1[1]._id);
            var ques3 = $scope.Answer.hasOwnProperty($scope.category1[2]._id);
            if (!ques1 || !ques2 || !ques3) {
                ...
                $scope.savingsHabitScore = getScore(ans);
                ...
                console.log("Final score" + $scope.savingsHabitScore);
                $scope.answers[$scope.category1[0].qsno - 1] = ans[0];
                $scope.answers[$scope.category1[1].qsno - 1] = ans[1];
                $scope.answers[$scope.category1[2].qsno - 1] = ans[2];
                $scope.scores.push({
                    "category": "Savings Habit",
                    "score": $scope.savingsHabitScore,
                    "commentary": $scope.savingsHabitDesctiption,
                    "service_1": $scope.description[0].service_1,
                    "service_2": $scope.description[0].service_2
                });
        ...
        if (questions === "contingencyPreparedness") {
            var ques1 = $scope.Answer.hasOwnProperty($scope.category2[0]._id);
            var ques2 = $scope.Answer.hasOwnProperty($scope.category2[1]._id);
            var ques3 = $scope.Answer.hasOwnProperty($scope.category2[2]._id);
            if (!ques1 || !ques2 || !ques3) {
                toaster.error({body: "Please answer the question"});
            } else {
                var ans = [];
                ...
                $scope.contingencyPreparednessScore = getScore(ans);
                ...
                $scope.answers[$scope.category2[0].qsno - 1] = ans[0];
                $scope.answers[$scope.category2[1].qsno - 1] = ans[1];
                $scope.answers[$scope.category2[2].qsno - 1] = ans[2];
                $scope.scores.push({
                    "category": "Contingency Preparedness",
                    "score": $scope.contingencyPreparednessScore,
                    "commentary": $scope.contingencyDesctiption,
                    "service_1": $scope.description[1].service_1,
                    "service_2": $scope.description[1].service_2
                });
        ...
        if (questions === "loansituation") {
            var ques1 = $scope.Answer.hasOwnProperty($scope.category3[0]._id);
            var ques2 = $scope.Answer.hasOwnProperty($scope.category3[1]._id);
            var ques3 = $scope.Answer.hasOwnProperty($scope.category3[2]._id);
            if (!ques1 || !ques2 || !ques3) {
                toaster.error({body: "Please answer the question"});
            } else {
                var ans = [];
                ...
                $scope.loansituationScore = getScore(ans);
                ...
                $scope.answers[$scope.category3[0].qsno - 1] = ans[0];
                $scope.answers[$scope.category3[1].qsno - 1] = ans[1];
                $scope.answers[$scope.category3[2].qsno - 1] = ans[2];
                $scope.scores.push({
                    "category": "Loan situation",
                    "score": $scope.loansituationScore,
                    "commentary": $scope.loansituationDesctiption,
                    "service_1": $scope.description[2].service_1,
                    "service_2": $scope.description[2].service_2
                });
        ...
        if (questions === "investmentDiscipline") {
            var ques1 = $scope.Answer.hasOwnProperty($scope.category4[0]._id);
            var ques2 = $scope.Answer.hasOwnProperty($scope.category4[1]._id);
            var ques3 = $scope.Answer.hasOwnProperty($scope.category4[2]._id);
            if (!ques1 || !ques2 || !ques3) {
                toaster.error({body: "Please answer the question"});
            } else {
                var ans = [];
                ...
                $scope.investmentDisciplineScore = getScore(ans);
                ...
                $scope.answers[$scope.category4[0].qsno - 1] = ans[0];
                $scope.answers[$scope.category4[1].qsno - 1] = ans[1];
                $scope.answers[$scope.category4[2].qsno - 1] = ans[2];
                $scope.scores.push({
                    "category": "Investment Discipline",
                    "score": $scope.investmentDisciplineScore,
                    "commentary": $scope.investmentDesctiption,
                    "service_1": $scope.description[3].service_1,
                    "service_2": $scope.description[3].service_2
                });
        ...
        if (questions === "generalAwareness") {
            var ques1 = $scope.Answer.hasOwnProperty($scope.category5[0]._id);
            var ques2 = $scope.Answer.hasOwnProperty($scope.category5[1]._id);
            var ques3 = $scope.Answer.hasOwnProperty($scope.category5[2]._id);
            if (!ques1 || !ques2 || !ques3) {
                toaster.error({body: "Please answer the question"});
            } else {
                var ans = [];
                ...
                $scope.generalAwarenessScore = getScore(ans);
                ...
                $scope.answers[$scope.category5[0].qsno - 1] = ans[0];
                $scope.answers[$scope.category5[1].qsno - 1] = ans[1];
                $scope.answers[$scope.category5[2].qsno - 1] = ans[2];
                $scope.scores.push({
                    "category": "General Awareness",
                    "score": $scope.generalAwarenessScore,
                    "commentary": $scope.generalAwarenessDesctiption,
                    "service_1": $scope.description[4].service_1,
                    "service_2": $scope.description[4].service_2
                });
...
function clientsCtrl($scope, $http, toaster, $rootScope) {
    $http({
        method: 'GET',
        ...
        response.data.forEach(function (user) {
            ...
            user.last=moment(user.createdAt).format('YYYY-MM-DD');
        });
        $scope.users = response.data;
    }, function errorCallback(response) {

    });
    $scope.loadUserProfile = function (_userid, firstname, lastname) {
        window.currentUesrId = _userid;
        window.currentUserFullName = firstname + " " + lastname;
    ...
    $scope.deleteUserProfile=function(_userid,firstname,lastname){
        $http({
            method: 'DELETE',
            url:  window.link.deleteProfile +'/'+_userid,
            headers : {'x-access-token': window.localStorage.getItem('token')}
        }).then(function successCallback(response) {
            toaster.success({body: firstname + lastname +" Profile Deleted!"});
        }, function errorCallback(response) {
...
angular.module('finatwork').controller('clientsCtrl', clientsCtrl);
...
function riskProfileCtrl($scope, $state, $http, toaster, c3ChartService, $uibModal) {
    ...
    $http({
        method: 'GET',
        url: window.link.riskProfileResult + "/" + window.localStorage.getItem("userid"),
        headers: {'x-access-token': window.localStorage.getItem('token')}
    }).then(function successCallback(response) {
        if (response.data) {
 ...
 */
function goalPortfolioDashboardCtrl($scope, $state, $http, c3ChartService, thousandSeparator, toaster) {
    $scope.thousandseparator = thousandSeparator.thousandSeparator;
    $scope.gridData = [];
    ...
    $scope.assedAllocationID = 'assedAllocationID';
    ...
    function showAMCChart(){
        /* Grid Data */
        for(var i in $scope.fundAll){
            var schemeName = getFundName($scope.fundAll[i]._schemeId);
            var folio = $scope.fundAll[i].folio;

            for (var k=0; k<$scope.fundAll[i].trxn.length; k++) {
                $scope.fundAll[i].trxn[k].tradDate =  moment($scope.fundAll[i].trxn[k].tradDate).format('DD-MM-YYYY');
                $scope.fundAll[i].trxn[k].amount =  Math.round($scope.fundAll[i].trxn[k].units * $scope.fundAll[i].trxn[k].NAV);
                $scope.fundAll[i].trxn[k].trxnType =  $scope.trxnDisplayType[$scope.fundAll[i].trxn[k].trxnType];

            }
            var subFund = $scope.fundAll[i].trxn;
            var absReturn = ($scope.fundAll[i].absReturn * 100).toFixed(2);
            var xir = ($scope.fundAll[i].xirr * 100).toFixed(2);
            var currentValue = $scope.thousandseparator(Math.round($scope.fundAll[i].marketValue));
            var investedAmt = $scope.thousandseparator(Math.round($scope.fundAll[i].investedCost));
            var profit = $scope.thousandseparator(Math.round($scope.fundAll[i].marketValue) - Math.round($scope.fundAll[i].investedCost));
            var goalType = $scope.goalsDisplayName[$scope.fundAll[i].type];
            $scope.gridData.push({Type:goalType,Category:$scope.fundAll[i].category,Scheme_Name:schemeName,Folio:folio,Invested_Amt:investedAmt,Current_Value:currentValue,Profit:profit, Absolute_Returns:absReturn,XIRR:xir,sunFund:subFund})
        }
        for(i = 0; i < $scope.gridData.length; i++){
            $scope.gridData[i].subGridOptions = {
                columnDefs: [ {name:"Date", field:"tradDate"}, {name:"Trxn Type", field:"trxnType"},{name:"Units", field:"units"},{name:"NAV", field:"NAV"},{name:"Amount", field:"amount"}],
                data: $scope.gridData[i].sunFund
            }
        }
        ...
        $scope.fundAll.forEach( function( item ) {
            var grade = fundObject[item._schemeId] = fundObject[item._schemeId] || "";
        });
        //console.log( JSON.stringify( fundObject, null, 4 ) );
        var sortFund = Object.keys(fundObject).map(function(k) { return k });
        var sortAMC = [],amcNamewithSchemeid = [];
        sortFund.forEach( function( item ) {
            var amcName = getAMCName(item);
            amcNamewithSchemeid.push({_schemeId:item,amcname:amcName});
            if(sortAMC.indexOf(amcName)=== -1)
                sortAMC.push(amcName);
        });

        var columns = [];
        for(var i in amcNamewithSchemeid){
            var tempArry = [];
            tempArry.push(amcNamewithSchemeid[i].amcname);
            for(var j in $scope.fundAll) {
                if(amcNamewithSchemeid[i]._schemeId === $scope.fundAll[j]._schemeId)
                    tempArry.push($scope.fundAll[j].marketValue);
            }
            columns.push(tempArry);
            tempArry = [];
        }
    ...
    function getFundData(){
        ...
        $http({
            method: 'GET',
            url:  window.link.create_goal+"?state=active&view=goal+fund+trxn&userid="+userId,
            headers : { 'x-access-token': window.localStorage.getItem('token') }
        }).then(function successCallback(response) {
            if (response.data.length > 0) {
                ...
                $scope.fundAll = [];
                for (var i in $scope.goalData) {
                    for (j in $scope.goalData[i].funds) {
                        $scope.goalData[i].funds[j].type = $scope.goalData[i].type;
                    }
                    $scope.fundAll = $scope.fundAll.concat($scope.goalData[i].funds);
                }

                var fundObject = {};
                $scope.fundAll.forEach(function (item) {
                    var grade = fundObject[item.category] = fundObject[item.category] || "";
                });
                //console.log(JSON.stringify(fundObject, null, 4));
                var sortFund = Object.keys(fundObject).map(function (k) {
                    return k
                });

                var assetColumns = [];
                for (var i in sortFund) {
                    var tempArry = [];
                    tempArry.push(sortFund[i]);
                    for (var j in $scope.fundAll) {
                        if (sortFund[i] === $scope.fundAll[j].category)
                            tempArry.push($scope.fundAll[j].marketValue);
                    }
                ...
                for (var i in $scope.goalData) {
                    var tempArry = [];
                    tempArry.push($scope.goalData[i].type);
                    for (var j in $scope.goalData[i].funds)
                        tempArry.push($scope.goalData[i].funds[j].marketValue);
...
angular.module('finatwork').controller('goalPortfolioDashboardCtrl',['$scope', '$state', '$http' , 'c3ChartService', 'thousandSeparator', 'toaster', goalPortfolioDashboardCtrl]);
    ...
    function fourmStatus() {
        ...
        $http({
            ...
            try {
                ...
                var status = Object.keys($scope.userDefaultStatus).map(function (k) {
                    return k
                });
                var sortStatus = [];
                ...
                for (var i = 0; i < userStatus.length; i++) {
                    for (var j = 0; j < sortStatus.length; j++) {
                        if (sortStatus[j].status.toLowerCase() === userStatus[i].status) {
                            sortStatus[j].isactive = true;
                            sortStatus[j].date = moment(userStatus[i].date, "YYYY-MM-DD").format('YYYY-MM-DD');
                        }
                    }
                }
...
function nscHandOverCtrl($http, $scope, $state) {
    ...
    ($scope.IsSipAvailable === "true")? $scope.dynamicStep4 = "Step 4:" : $scope.dynamicStep4 = "Step 3:";
            ...
            try {
                ...
                for (var i=0; i<userStatus.length; i++) {
                    sortStatus.push({
                        ...
                        date: moment(userStatus[i].date, "YYYY-MM-DD").format('YYYY-MM-DD'),
                        isactive:false
                    });
                    for(var j = 0; j<userStatus[i].notes.length; j++){
                        notes.push({
                            ...
                            date: moment(userStatus[i].notes[j].date,"YYYY-MM-DD").format('YYYY-MM-DD'),
 ...
 */
function transcationController($scope, $http, $state) {
    $http({
        method: 'GET',
        url: window.link.create_goal + "?state=transect&view=goal",
        headers: {'x-access-token': window.localStorage.getItem('token')}
    }).then(function successCallback(response) {
        response.data.forEach(function (goal) {
...
/**
 * Created by Finatwork on 21-02-2017.
 */
...
function serviceCartController($scope, $state, $http,$window,toaster,ModalService) {
    ...
    $scope.checkout = function () {
        ...
        angular.forEach($scope.fincarts, function (obj, index) {
            if (obj.selected == true && obj.offerPrice > 0) {
                data.svcPurchased.push({name:obj.name,amount:obj.offerPrice, validity:moment().day(obj.validity)});
            }
        });
...
function pgResponseController($scope, $state, $stateParams, $http) {
    $scope.init = function () {
