...
function goalPortfolioDashboardCtrl($scope, $state, $http, c3ChartService, thousandSeparator, toaster) {
    ...
    $scope.gridOptions.columnDefs = [
        ...
        { name: 'Mutual Fund House',field:"AMC_Name",width: '13%'},
    ...
    function showAMCChart(){
        ...
        for(var i in $scope.fundAll){
            ...
            var amcName = getAMCName($scope.fundAll[i]._schemeId);

            for (var k=0; k<$scope.fundAll[i].trxn.length; k++) {
                $scope.fundAll[i].trxn[k].orderDate =  moment($scope.fundAll[i].trxn[k].orderDate, "YYYY-MM-DD").format('YYYY-MM-DD');
            }
            ...
            $scope.gridData.push({Type:goalType,Category:$scope.fundAll[i].category,Scheme_Name:schemeName,AMC_Name:amcName,Invested_Amt:investedAmt,Current_Value:currentValue,Profit:profit, Absolute_Returns:absReturn,XIRR:xir,sunFund:subFund})
        }
        for(i = 0; i < $scope.gridData.length; i++){
            $scope.gridData[i].subGridOptions = {
                columnDefs: [ {name:"Trxn No", field:"trxnNo"},{name:"Units", field:"units"},{name:"Order Date", field:"orderDate"},{name:"Status", field:"status"}],
